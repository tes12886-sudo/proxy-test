...
# Konfigurasi Gamevar Custom (Termasuk float 8.5)
CUSTOM_GAMEVAR = (
...
# Template Data Versi Free Fire Biasa (Prefix 1.x)
VER_DATA_BIASA = {
    "code": 0,
    "is_server_open": True,
...
# Template Data Versi Free Fire MAX (Prefix 2.x)
VER_DATA_MAX = {
    "code": 0,
    "is_server_open": True,
    "is_firewall_open": False,
...
async def catch_all(request: Request, path: str):
    ...
    if "ver.php" in path.lower() or path.lower().endswith("ver.php"):
        client_version = request.query_params.get("version", "1.130.22")
        major_prefix = client_version.split(".")[0]

        # Pilih struktur data template berdasarkan prefix versi
        if major_prefix == "2":
            response_data = VER_DATA_MAX.copy()
            response_data["remote_version"] = f"2.130.22"
        else:
            response_data = VER_DATA_BIASA.copy()
            response_data["remote_version"] = f"1.130.22"
